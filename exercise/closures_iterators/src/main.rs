...
fn main() {
    ...
    for x in &mut numbers {//or numbers.iter_mut()
    ...
    // SOLUTION 2
    let mut numbers = vec![1, 2, 3, 4];
    let formatted: Vec<_> = numbers.iter_mut().map(|x| (*x*3)).collect();
    println!("{:?}", formatted); // should print [3, 6, 9, 12]
